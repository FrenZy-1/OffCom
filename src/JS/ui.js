...
async function showScreen(screenId) {
	...
	if (!document.getElementById(screenId)) {
		try {
			...
			const fileName =
				screenId.replace(/([A-Z])/g, "-$1").toLowerCase() + ".html";
			log("[UI] Fetching dynamically:", fileName);

			const response = await fetch(`/src/HTML/${fileName}`);
...
function refreshCallScreen() {
	...
	document
		.querySelectorAll(".transmit-btn")
		.forEach((b) => b.classList.remove("active"));
	document
		.getElementById("transmit-" + transmitMode)
		?.classList.add("active");
	document
		.getElementById("voxSettings")
		?.classList.toggle("hidden", transmitMode !== "vox");
	document
		.getElementById("pttContainer")
		?.classList.toggle("hidden", transmitMode !== "ptt");
...
function updateUsername(val) {
	myUsername =
		val.trim() || "Rider-" + Math.floor(1000 + Math.random() * 9000);
...
function updateCallStatus() {
	const connectedCount = Object.values(peers).filter(
...
function updateHostUI() {
	if (
		isHost &&
		document.getElementById("screenHost")?.classList.contains("active")
	) {
...
// ---- Lazy overlay loader ----
// Overlays/modals live in /src/HTML/ partials and are injected into #overlayContainer on first use.
...
async function loadOverlay(filename) {
	...
	try {
		const res = await fetch(`/src/HTML/${filename}`);
...
async function loadSettingsScreen() {
	...
	try {
		const response = await fetch("/src/HTML/screen-settings.html");
		const html = await response.text();
...
async function openSettings() {
	...
	if (
		document
			.getElementById("dynamicScreenContainer")
	...
	const active = document.querySelector(
		".screen.active:not(#dynamicScreenContainer)",
	);
...
function populateSettingsValues() {
	...
	if (dlLogsBtn)
		dlLogsBtn.style.display = isFileLoggingEnabled ? "block" : "none";
	// ----------------------------------------------

	const micGain = micGainNode
		? micGainNode.gain.value
		: parseFloat(localStorage.getItem("offcom_mic_gain") || "1");
	const speakerGain = speakerGainNode
		? speakerGainNode.gain.value
		: parseFloat(localStorage.getItem("offcom_speaker_gain") || "1");
...
function addPeerToUI(peerObj) {
	...
	const label =
		peerObj.role === "host" ? `${peerObj.name} (Host)` : peerObj.name;
...
async function openPeerModal(peerId) {
	...
	localMuteBtn.textContent = p.locallyMuted
		? "🔊 Unmute Locally"
		: "🔇 Mute Locally";
	...
	if (isHost) {
		...
		roomMuteBtn.textContent = p.globallyMuted
			? "🎙️ Unmute for Room"
			: "🔕 Mute for Room";
		roomMuteBtn.className = p.globallyMuted
			? "btn btn-secondary"
			: "btn btn-warning";
	} else {
	...
	document.getElementById("peerModalKickBtn").style.display = isHost
		? ""
		: "none";
	...
	overlay.onclick = (e) => {
		if (e.target === overlay) closePeerModal();
	};
}
...
function togglePeerMute() {
	...
	const badge = document.getElementById(
		"peer_badge_" + currentSelectedPeerId,
	);
	if (badge) {
	...
	showToast(
		p.locallyMuted ? `${p.name} muted on your device` : `${p.name} unmuted`,
		false,
	);
	closePeerModal();
}
...
function muteForRoom() {
	...
	showToast(
		p.globallyMuted
			? `${p.name} muted for the room`
			: `${p.name} unmuted for the room`,
		false,
	);
	closePeerModal();
}
...
function toggleFileLogging(enabled) {
	...
	document.getElementById("downloadLogsBtn").style.display = enabled
		? "block"
		: "none";
	showToast(
		enabled ? "File logging enabled" : "File logging disabled",
		false,
	);
}
